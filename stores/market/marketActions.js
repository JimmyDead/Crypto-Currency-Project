...
export const getHoldingFailure = (erro) => ({
...
export function getHoldings(holdings = [], currency = 'usd', orderBy = 'market_cap_desc',
    ...
    return dispatch => {
        ...
        return axios({
            ...
            if (response.status === 200) {
                let myHoldings = response.data.map((item) => {
                    ...
                    //recuperar nossas participações atuais, quantidade atual
                    let coin = holdings.find(a => a.id === item.id)

                    //preco dos ultimos 7 dias 
                    let priceSevenDays = item.current_price / (1 + item.price_change_percentage_7d_in_currency * 0.01)
                    ...
                    return {
                        ...
                        holding_value_change_7d: (item.current_price - priceSevenDays) * coin.qty,
                        sparkline_in_7d: {
        ...
        }).catch((erro) => { dispatch(getHoldingFailure(erro)) })
    }
}
